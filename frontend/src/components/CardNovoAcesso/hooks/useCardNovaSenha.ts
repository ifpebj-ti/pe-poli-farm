...
export function useCardNovaSenha() {
  ...
  const submitForm: SubmitHandler<typeMyschema> = async (data) => {
    try {
      ...
      const response = await NewPassword(data.email, data.password, data.accessCode); 
      ...
      if (error instanceof AxiosError) {
        if (error.response) {
          ...
          if (statusCode === 400 && apiError?.errors) {
            ...
            errorMessage = apiError?.message || apiError?.title || 'Código de acesso inválido ou expirado.';
          ...
          toast.error('Erro interno ao tentar alterar a senha. Tente novamente mais tarde.');
        }
      } else {
